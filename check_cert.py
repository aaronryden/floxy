...
async def check_certificates_periodically():
    while True:
            ...
            if os.path.exists(cert_path):
                try:
                    with open(cert_path, "rb") as cert_file:
                        ...
                        print(f"✅ {domain} cert valid until {not_after} ({days_left} days left)")

                        if days_left < 10:
                            print(f"⚠️  Certificate for {domain} expires in {days_left} days! Consider renewing.")
                except Exception as e:
                    print(f"❌ Failed to parse cert for {domain}: {e}")
            else:
                print(f"❌ No certificate found for {domain} at {cert_path}")

        await asyncio.sleep(86400)  # wait 24 hours before next check